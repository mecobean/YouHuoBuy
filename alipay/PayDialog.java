package cn.bjsxt.youhuo.alipay;
...
public class PayDialog extends BaseDialog implements View.OnClickListener {
    ...
    private void initListener() {
        ...
        h5pay.setOnClickListener(this);
    }

    @Override
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.dialog_pay_pay://支付宝支付
                pay();
                break;
            case R.id.dialog_pay_h5pay://网页支付
                h5Pay();
                break;
            case R.id.dialog_pay_back://返回
                dismiss();
                break;
        }
    }
    private void initDialog() {
        setContentView(rootView, activity.getWindowManager().getDefaultDisplay().getWidth(), WindowManager.LayoutParams.WRAP_CONTENT);
        //防止出现 黑边
    ...
    private void initViews() {
        rootView = inflater.inflate(R.layout.dialog_pay, null);
        back = (ImageView) rootView.findViewById(R.id.dialog_pay_back);
        pay = (Button) rootView.findViewById(R.id.dialog_pay_pay);
        h5pay = (Button) rootView.findViewById(R.id.dialog_pay_h5pay);
    ...
    public void pay() {
        if (TextUtils.isEmpty(PARTNER) || TextUtils.isEmpty(RSA_PRIVATE) || TextUtils.isEmpty(SELLER)) {
            ToastUtils.toast("警告: 需要配置PARTNER | RSA_PRIVATE| SELLER");
            return;
        }
